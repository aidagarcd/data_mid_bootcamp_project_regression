...
CREATE TABLE house_price_data (
    ...
    zipcode INT,
    lat DECIMAL(9,6),
    `long` DECIMAL(9,6), -- Encapsulamos `long` entre comillas invertidas
    sqft_living15 INT,
    sqft_lot15 INT,
    price INT
);

-- importig the data from csv file with Tabla Data Import Wizard (boton derecho en tablas, 5 table data impoer wizard)
...
-- step calculate the avg per typology
select bedrooms, avg(price) as avg_price_per_bedrooms from house_price_data
where bedrooms in (3, 4)
group by bedrooms;

-- calculate the difference in average prices between the two typologies.
select
